 ...
 *
 */
public class AutoDriveShootCommand extends CommandBase {
...
//            } else {
                robotToSample1 = cmd.buildTraj( tb -> tb.splineTo( new Vector2d( 36, 34 * bluePlus), blueAlliance ? 45 : -45).build() );
//            }

        Command robotToSquare = cmd.buildTraj( tb -> tb.lineToSplineHeading( new Pose2d( 50, 15 * bluePlus, 0) ).build() );
    ...
    private Pose2d getStartingPose() {
        ...
        double x = -63.0;
        double y = 31.0 * bluePlus;

        // heading depends on if red or blue
        double heading = Math.toRadians( color == BLUE ? 0 : 0 );
